...
public class Controller : MonoBehaviour {
	...
	private void Atack(float direction) {
		Physics2D.Raycast (cachedTransform.position, new Vector2 (direction, 0), atackRadius, 1 << LayerMask.NameToLayer ("Enemy"));
	}
	...
	private void Update () {
		...
		if (human) {
			...
			Debug.Log(Physics2D.Raycast (cachedTransform.position, Vector2.down, 6f, 1 << LayerMask.NameToLayer ("Ground")));
